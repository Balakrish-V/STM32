...
// Clock bit macros for GPIO ports in STM32F4 series (AHB1ENR)
...
// Custom GPIO and RCC type definitions for STM32F4
typedef struct {
...
typedef struct {
    ...
    volatile uint32_t CR;       // RCC control register
    volatile uint32_t CFGR;     // RCC configuration register
} MY_RCC_TypeDef;
...
// Macro to enable GPIO clock for STM32F4 (using AHB1ENR for STM32F4)
#define ENABLE_GPIO_CLOCK(port, clk_bit) \
    (RCC->AHB1ENR |= (1 << (clk_bit))); \
